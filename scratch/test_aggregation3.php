...
// Stats Query with JOIN with unique product subquery
$stats_query = "
    SELECT 
    ...
    FROM (
        ...
        LEFT JOIN (
            ...
            INNER JOIN (SELECT DISTINCT product_id FROM ims_product WHERE " . $searchQuery . ") ip ON sb.SKU_CODE = ip.product_id
        ...
        LEFT JOIN (
            ...
            INNER JOIN (SELECT DISTINCT product_id FROM ims_product WHERE " . $searchQuery . ") ip ON sc.SKU_CODE = ip.product_id
...
// Bind all params three times (for main query and two subqueries)
$bind_params = [];
...
// Let's rewrite the query to use unique parameter names for each subquery reference
$stats_query = str_replace("WHERE " . $searchQuery, "WHERE " . str_replace(":", ":st_", $searchQuery), $stats_query);
// Wait, the first one is the main, second is st, third is s.
// Let's do it manually to avoid mistakes:
